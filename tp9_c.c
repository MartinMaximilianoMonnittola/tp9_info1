// Link al repositorio https://github.com/MartinMaximilianoMonnittola/tp9_info1 
// legajo 422342
// Martin Monnittola
...
void imprimir_estudiante(struct estudiante e) {
  materia_t *materia = e.materia;
  printf("%10d", e.legajo);
  printf(" %s \n", e.nombre);

  for(int k = 0; k < e.cantidad_materias; k++) {
    printf(" %s ", (e.materia + k)->nombre);
    for (int i = 0; i < (e.materia + k)->cantidad_parciales; i++)
        printf("%5d", (e.materia + k)->parcial[i]);

    printf("  Promedio: %.2f\n", (e.materia + k)->promedio);
    printf("\n");
  }
}
...
void calcular_promedio(materia_t *materia) {
  float promedio = 0;
  int cant = materia->cantidad_parciales;
  int *lista_notas = materia->parcial;

  for(int i = 0; i < cant; i++) {
    promedio += (float)*(lista_notas + i) / cant;
  }
